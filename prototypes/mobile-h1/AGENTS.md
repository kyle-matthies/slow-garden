...
## Slow Garden product-specific direction

- Treat this as an interaction prototype for a native iOS-first product with a desktop web companion. The browser runtime is not the production client architecture.
- Preserve Meadow and Cabinet as two views of the same garden and the same bloom identity. Meadow is the open, living capture/exploration posture; Cabinet is the pressed, evidence-rich review posture.
- Keep the Flower-inspired sense of field, air, and movement in Meadow without copying game assets or letting atmosphere obscure readable source material.
- Keep source evidence visually prominent in Cabinet. At standard iPhone text sizes, pair the specimen and source-clipping column when all text remains legible; switch to a stacked layout for Dynamic Type or constrained widths.
- Capture must remain quiet and non-interruptive. AI does not answer while the person writes. Review actions are Keep, Correct, and Prune, and none may alter the original source notes.
